...
public class BlockLightningKeyBlock extends BlockBase implements IBlockUpdater, ITileEntityProvider {

    private final Item activationItem = ModItems.LIGHTNING_KEY;
    ...
    public BlockLightningKeyBlock(String name, BiFunction<World, BlockPos, Entity> spawnPortal) {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY,
        ...
        if (playerIn.getHeldItemMainhand().getItem() == this.activationItem && !worldIn.isRemote || playerIn.getHeldItemMainhand().getItem() instanceof ItemLightningKey && !worldIn.isRemote) {
