...
public class PlayerMeleeAttack {
    ...
    private static boolean onPlayerAttackTargetCustom(EntityPlayer player, Entity target) {
        ItemStack stack = player.getHeldItemMainhand();
        return stack.isEmpty() || !stack.getItem().onLeftClickEntity(stack, player, target);
    }

    public static void attackTargetEntityWithCurrentItemCustom(EntityPlayer player, Entity targetEntity) {
        if (!onPlayerAttackTarget(player, targetEntity))
            return;
        ...
        if (targetEntity.canBeAttackedWithItem()) {
            if (!targetEntity.hitByEntity(player)) {
                ...
                float atkCooldown = 1;
                damage = (damage * (0.2F + atkCooldown * atkCooldown * 0.8F)) * (ModUtils.addOffhandDualBonuses(player));
                bonusDamage = bonusDamage * atkCooldown;
                player.resetCooldown();

                if (damage > 0.0F || bonusDamage > 0.0F) {
                    boolean cooldownCharged = true;
                    boolean sprintAtk = false;
                    int knockback = 0;
                    knockback = knockback + EnchantmentHelper.getKnockbackModifier(player);
                    ...
                    boolean critical = player.fallDistance > 0.0F && !player.onGround && !player.isOnLadder() && !player.isInWater()
                            && !player.isPotionActive(MobEffects.BLINDNESS) && !player.isRiding() && targetEntity instanceof EntityLivingBase;
                    critical = critical && !player.isSprinting();
                    ...
                    if (!critical && !sprintAtk && player.onGround && d0 < player.getAIMoveSpeed()) {
                        sweepAttack = true;
                    }
    ...
    public static void attackTargetEntityWithCurrentItemRadius(EntityPlayer player, Entity targetEntity, float scale) {
        if (!onPlayerAttackTarget(player, targetEntity))
            return;

        if (targetEntity.canBeAttackedWithItem()) {
            if (!targetEntity.hitByEntity(player)) {
                float damage = (float) player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() * scale;
                float bonusDamage;
