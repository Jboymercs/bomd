package com.dungeon_additions.da.items.armor.nightfall;
...
public class ItemNightfallChestplate extends ItemArmor implements IHasModel {
    ...
    public ItemNightfallChestplate(String name, ArmorMaterial materialIn, int renderIdx, EntityEquipmentSlot slotIn, String info_loc) {
        ...
        this.kingSpeedBoost = new AttributeModifier("nightfallChestplateBoost", 0.03, 0);
    }
    ...
    public ItemNightfallChestplate setArmorBonusDesc(String armorBonusDesc) {
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (entityIn instanceof EntityPlayer) {
            ...
            if (player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == this) {
                if (player.hurtTime == 1 && !player.getCooldownTracker().hasCooldown(this) && !worldIn.isRemote) {
                    if(worldIn.rand.nextInt(10) == 0 && player.getAttackingEntity() != null) {
                        if(player.getAttackingEntity() instanceof EntityLivingBase) {
                            EntityLivingBase base = ((EntityLivingBase) player.getAttackingEntity());
                            float damage = 8 + ModUtils.addMageSetBonus(player, 0, 1.5F);
                            EntityDelayedLazer lazer = new EntityDelayedLazer(worldIn, 15, base.getPositionVector().add(0, 0.75F, 0), player, damage, base);
                            lazer.setPosition(player.posX, player.posY + player.getEyeHeight() + 1, player.posZ);
                            worldIn.spawnEntity(lazer);
                            player.getCooldownTracker().setCooldown(stack.getItem(), 20 * 20);
                        }
                    }
                }
            }
    ...
    public String getArmorTexture(ItemStack stack, Entity entity, EntityEquipmentSlot slot, String type) {
        return ModReference.MOD_ID + ":textures/models/armor/nightfall_chestplate.png";
    }
}
