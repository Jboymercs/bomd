...
public class WorldGenAspectForge extends WorldGenerator {

    private int spacing;
    private int separation;


    public WorldGenAspectForge() {
        this.spacing = WorldConfig.aspect_forge_spacing;
        this.separation = 16;
    }
    ...
    protected StructureStart getStructureStart(World world, int chunkX, int chunkZ, Random rand) {
        return new WorldGenAspectForge.Start(world, rand, chunkX, chunkZ);
    }
    ...
    public static class Start extends StructureStart {
        ...
        private void create(World worldIn, Random rnd, int chunkX, int chunkZ) {
            ...
            BlockPos ordinal_2 = new BlockPos(posI.getX() + 4, 0, posI.getZ() + 4);
            int y = getSurfaceHeight(worldIn, posI, WorldConfig.aspect_forge_min_y, WorldConfig.aspect_forge_max_y);
            int y2 = getSurfaceHeight(worldIn, ordinal_2, WorldConfig.aspect_forge_min_y, WorldConfig.aspect_forge_max_y);
            ...
            if(y != 0 && y2 != 0) {
                if(y2 < y + 2 || y2 > y - 2) {
                    for (int i = 0; i < 4; i++) {
                        ...
                        BlockPos blockpos = posI.add(-4, y - 2 + Math.abs(regulatedY), -4);
                        AspectForge arena = new AspectForge(worldIn, worldIn.getSaveHandler().getStructureTemplateManager(), components);
                        arena.startBuilding(blockpos, Rotation.NONE, "aspect_forge");
