...
public class WorldGenCultsDomain extends WorldGenerator {
    private int spacing;
    private int separation;

    public WorldGenCultsDomain() {
        this.spacing = WorldConfig.cult_castle_spacing; //112
        this.separation = 16;
    }

    protected StructureStart getStructureStart(World world, int chunkX, int chunkZ, Random rand) {
        return new WorldGenCultsDomain.Start(world, rand, chunkX, chunkZ);
    }
        ...
        private void create(World worldIn, Random rnd, int chunkX, int chunkZ) {
            ...
            //basically this is to help bring two points to a more common ground
                for (int i = 0; i < 4; i++) {
                    ...
                    CultsDomain castle = new CultsDomain(worldIn, worldIn.getSaveHandler().getStructureTemplateManager(), components);
                    castle.startDungeon(new BlockPos(posI.getX(), WorldConfig.cult_castle_y_height, posI.getZ()), Rotation.NONE);
