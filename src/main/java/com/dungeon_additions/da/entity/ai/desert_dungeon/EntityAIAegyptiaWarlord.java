package com.dungeon_additions.da.entity.ai.desert_dungeon;
...
public class EntityAIAegyptiaWarlord <T extends EntityAegyptianWarlord & IAttack> extends EntityAIBase {
    ...
    public EntityAIAegyptiaWarlord(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 40.0f);
    }

    public EntityAIAegyptiaWarlord(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!entity.lockLook && !entity.isJumping) {
            if(canSee) {
                if(distSq < 47) {
