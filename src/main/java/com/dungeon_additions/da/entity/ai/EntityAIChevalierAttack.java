...
public class EntityAIChevalierAttack <T extends EntityChevalier & IAttack> extends EntityAIBase {
    ...
    public EntityAIChevalierAttack(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityAIChevalierAttack(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!entity.lockLook) {
            if (distSq <= 2 && canSee || distSq <= 30 && this.entity.focusMode) {
                this.entity.getNavigator().clearPath();
                ++this.strafingTime;
            } else if (!this.entity.isRandomGetAway) {
