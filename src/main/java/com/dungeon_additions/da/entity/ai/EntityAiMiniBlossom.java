...
public class EntityAiMiniBlossom <T extends EntityMiniBlossom & IAttack> extends EntityAIBase {
    ...
    public EntityAiMiniBlossom(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityAiMiniBlossom(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void updateTask() {
        ...
        if(!this.entity.isFightMode()) {
            if(distSq >= this.maxAttackDistSq || distSq >= this.maxAttackDistSq/2 && this.entity.moveDelay < 0) {
                moveModified(target, distSq, canSee);
            }
        }


        this.entity.moveDelay--;


        if (distSq <= this.maxAttackDistSq && canSee && !this.entity.isTravel()) {
            this.attackTime--;
            if (this.attackTime <= 0 && !this.entity.isSprout() && !this.entity.isBurrow()) {
                this.attackTime = this.entity.startAttack(target, (float) distSq, this.strafingBackwards);
            }
        }
    }

    public void moveModified(EntityLivingBase target, double distSq, boolean canSee) {
        //Tells it to play the Animation on One Tick
        if(!this.entity.hasPlayedAnimation && !this.entity.isTravel()) {
            this.entity.playBurrowAnimation();
            this.entity.hasPlayedAnimation = true;
        }

       else if(distSq <= maxAttackDistSq/2 && !this.entity.hasPlayedAnimation || distSq <= maxAttackDistSq && canSee && !this.entity.hasPlayedAnimation) {
            this.entity.playSproutAnimation();
            this.entity.hasPlayedAnimation = true;
            //Keeps the Entity from pursuing states quickly

        } else if(this.entity.isTravel()){
