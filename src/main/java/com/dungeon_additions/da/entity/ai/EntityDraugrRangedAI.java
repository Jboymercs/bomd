...
public class EntityDraugrRangedAI  <T extends EntityDraugrRanger & IAttack> extends EntityAIBase {
    ...
    public EntityDraugrRangedAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityDraugrRangedAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!this.entity.lockLook) {
            if(distSq <= 25 && canSee && !this.entity.isRangedAttack()) {
