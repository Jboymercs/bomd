...
public class EntityDraugrEliteAttackAI <T extends EntityEliteDraugr & IAttack> extends EntityAIBase {
    ...
    public EntityDraugrEliteAttackAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityDraugrEliteAttackAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!this.entity.lockLook && !this.entity.isDeathState()) {
            if (distSq <= 9 && canSee) {
