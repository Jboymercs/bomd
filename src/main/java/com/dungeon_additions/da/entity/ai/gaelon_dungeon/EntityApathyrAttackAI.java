package com.dungeon_additions.da.entity.ai.gaelon_dungeon;
...
public class EntityApathyrAttackAI  <T extends EntityApathyr & IAttack> extends EntityAIBase {
    ...
    private static final float STRAFING_DIRECTION_CHANGE_CHANCE = 0.2f;

    public EntityApathyrAttackAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityApathyrAttackAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
             if(!entity.lockLook && !this.entity.isDeathState()) {
                 if (!this.entity.isJumpAway()) {
                     if (canSee) {
                         if (this.entity.flagForStrafing) {
                             this.entity.getNavigator().clearPath();
                             this.entity.faceEntity(target, this.lookSpeed + 10, this.lookSpeed);
                             this.strafingTime++;
                         } else if (distSq < 64) {
                             //have entity backaway
                             this.entity.getMoveHelper().strafe(-1 * strafeAmount, 0.2F);
                             this.entity.faceEntity(target, this.lookSpeed, this.lookSpeed);
                             if(distSq < 25 && !this.entity.isFightMode()) {
                                 EntityLivingBase targetI = this.entity.getAttackTarget();
                                 if(targetI != null) {
                                     double d0 = (this.entity.posX - targetI.posX) * 0.011;
                                     double d1 = (this.entity.posY - targetI.posY) * 0.005;
                                     double d2 = (this.entity.posZ - targetI.posZ) * 0.011;
                                     this.entity.addVelocity(d0, d1, d2);
                                 }
                             }
                         } else {
                             this.entity.getNavigator().tryMoveToEntityLiving(target, this.moveSpeedAmp * 1.25);
                             this.strafingTime = -1;
                         }
