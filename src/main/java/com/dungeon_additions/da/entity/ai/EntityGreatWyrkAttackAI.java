...
public class EntityGreatWyrkAttackAI <T extends EntityGreatWyrk & IAttack> extends EntityAIBase {
    ...
    public EntityGreatWyrkAttackAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityGreatWyrkAttackAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!this.entity.lockLook && !this.entity.roll_state_active) {
            if (distSq <= 36 && canSee) {
