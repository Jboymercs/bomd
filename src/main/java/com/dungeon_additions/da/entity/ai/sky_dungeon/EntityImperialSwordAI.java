...
public class EntityImperialSwordAI <T extends EntityImperialSword & IAttack> extends EntityAIBase {
    ...
    public EntityImperialSwordAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityImperialSwordAI(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!entity.lockLook) {
            if(this.entity.exhaustionLevel > 1.0F && distSq <= 25) {
                this.entity.getNavigator().clearPath();
                ++this.strafingTime;
            } else if (distSq <= 9 && canSee) {
                this.entity.getNavigator().clearPath();
