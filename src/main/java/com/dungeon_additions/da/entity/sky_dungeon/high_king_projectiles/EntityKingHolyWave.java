...
public class EntityKingHolyWave extends Projectile {
    protected static final float AREA_FACTOR = 0.5f;

    public static final int PARTICLE_AMOUNT = 1;
    protected int updates = 5;

    public EntityKingHolyWave(World worldIn, EntityLivingBase throwerIn, float damage) {
        super(worldIn, throwerIn, damage);
        this.setNoGravity(true);
        this.setSize(0.25F, 1.7F);
    }

    public EntityKingHolyWave(World worldIn) {
        super(worldIn);
        this.setNoGravity(true);
        this.setSize(0.25F, 1.7F);
    }

    public EntityKingHolyWave(World worldIn, double x, double y, double z) {
        super(worldIn, x, y, z);
        this.setNoGravity(true);
        this.setSize(0.25F, 1.7F);
    }
        ...
        if (block.isFullCube()) {
            ...
            for (int i = 0; i < this.PARTICLE_AMOUNT; i++) {
                ...
                for (float y = 0; y < height; y += 0.2f) {
                    ...
                    ParticleManager.spawnDust(world, pos, ModColors.YELLOW, Vec3d.ZERO, ModRand.range(60, 80));
                }
