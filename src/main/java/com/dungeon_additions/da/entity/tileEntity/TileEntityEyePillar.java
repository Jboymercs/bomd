...
public class TileEntityEyePillar extends TileEntity implements ITickable {
    @Override
    public void update() {
        if(!this.world.isRemote) {

            AxisAlignedBB box = new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1);
            List<EntityApathyr> nearbyBoss = this.world.getEntitiesWithinAABB(EntityApathyr.class, box.grow(40D), e -> !e.getIsInvulnerable());
            if (!nearbyBoss.isEmpty()) {
                for (EntityApathyr blossom : nearbyBoss) {
